var app = (function () {
    ...
    let lightAngle = 0; // Startwinkel
    const lightSpeed = 0.005; // Geschwindigkeit der Bewegung
    const lightRadius = 5.0; // Radius der Kreisbahn
    let lightMoving = false; // Flag, um die Bewegung zu steuern
    ...
    function init() {
        ...
        initEventHandler();
        initPipline();
        initLightPositions(); // NEU: Lichtpositionen vorab setzen
    }

    function initLightPositions() {
        // Setze den Startwinkel für Licht 1 und Licht 2
        lightAngle = Math.PI / 4; // Setze den globalen Startwinkel auf 45 Grad
        const startAngle1 = Math.PI / 4; // 45 Grad für Licht 1
        const startAngle2 = startAngle1 + Math.PI; // 45 Grad + 180 Grad für Licht 2

        // Berechne die Positionen auf der Kreisbahn
        ...
        // Setze die initialen Lichtpositionen
        illumination.light[0].position = [x1, 2.0, z1]; // Licht 1
        illumination.light[1].position = [x2, 2.0, z2]; // Licht 2

        console.log(
            "Initiale Licht 1 Position:",
            illumination.light[0].position
        );
        console.log(
            "Initiale Licht 2 Position:",
    ...
    function initEventHandler() {
        ...
        window.addEventListener("keydown", function (event) {
            if ((event.key === "t" || event.key === "T") && !event.shiftKey) {
                ...
                console.log(
                    toonShadingEnabled
                        ? "Toon-Shading eingeschaltet"
                        : "Toon-Shading ausgeschaltet"
                );
            }
        });

        // Toggle für die Lichtbewegung bei Taste 'L'
        window.addEventListener("keydown", function (event) {
            if ((event.key === "l" || event.key === "L") && !event.shiftKey) {
                lightMoving = !lightMoving;
                console.log(
                    lightMoving
                        ? "Lichtbewegung gestartet"
                        : "Lichtbewegung gestoppt"
                );
            }
        });
    }
    ...
    function render() {
        ...
        // Aktualisiere die Lichtpositionen nur, wenn sie sich bewegen sollen
        if (lightMoving) {
            lightAngle += lightSpeed;

            // Berechne die neuen Lichtpositionen
        ...
        gl.uniform1i(prog.toonShadingUniform, toonShadingEnabled ? 1 : 0);
        ...
        // Animation Frame für kontinuierliches Rendern.
